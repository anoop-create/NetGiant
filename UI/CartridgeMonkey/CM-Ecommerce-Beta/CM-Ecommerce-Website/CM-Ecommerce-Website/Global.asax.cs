...
namespace CM_Ecommerce_Website
{
    public class MvcApplication : System.Web.HttpApplication
    {
        ...
        protected void Session_Start()
        {
            try
            {
                ...
                HttpContext.Current.Session["U_IsMobile"] = false;
                if ((b.IsMatch(u) || v.IsMatch(u.Substring(0, 4))))
                {
                    HttpContext.Current.Session["U_IsMobile"] = true;
                }

                string landingPage = HttpContext.Current.Request.Path;
                if (HttpContext.Current.Request.QueryString.ToString().Contains("gclid") 
                    || HttpContext.Current.Request.QueryString.ToString().Contains("pk_medium=cpc"))
                {
                    HttpContext.Current.Session["U_CampaignSource"] = "PPC";
                    HttpContext.Current.Session["U_Campaign"] = HttpContext.Current.Request.QueryString["pk_campaign"];
                    if (landingPage != "/")
                    {
