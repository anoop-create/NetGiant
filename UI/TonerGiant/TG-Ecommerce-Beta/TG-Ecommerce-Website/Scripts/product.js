...
$(function () {
    ...
    if (isCurrentPage('/product/')) {
        ...
        $('.jspContainer').each(function () {
            $(this).height($(this).outerHeight() + 100);
        });
        ...
        $('#imageModal').on('show.bs.modal',
            function (event) {
                ...
                $('.image-modal .image-slides img').each(function (index, elem) {
                    if ($(this).attr('data-image') === clickedImage) {
        ...
        $(document).on('click',
            ...
            function () {
                var imageUrl = $('img', this).attr('data-zoom');
        ...
        $(document).on('click',
            ...
            function () {
                var container = $(this).closest('.image-modal');
                var previousImage = container.find('.image-slide-container.active').prev('.image-slide-container');
                if (previousImage.length > 0) {
                    $('.image-modal .image-slide-container').removeClass('active');
                    previousImage.addClass('active');
                    if ($('.modal-dialog').is(":visible")) {
                        container.find('.large-image').attr('src', previousImage.find('img').attr('data-zoom'));
                    } else {
                        $('#prd-image').attr('src', previousImage.find('img').attr('data-image'));
                        $('#prd-image').attr('data-zoom', previousImage.find('img').attr('data-zoom'));
                    }
                }

                if (container.closest('.image-carousel').length > 0) {
                    var offsetEl = previousImage.prop('offsetLeft');
                    var offsetP = previousImage.parent().prop('offsetLeft');
                    if (offsetEl > offsetP) {
                        var scroller = 83;
                        var right = parseInt(previousImage.parent().css('right')) - scroller;
                        if (right >= 0) {
                            previousImage.parent().css('right', right + 'px');
                        }
                    }
                }
            });
        ...
        $(document).on('click',
            ...
            function () {
                var container = $(this).closest('.image-modal');
                var nextImage = container.find('.image-slide-container.active').next('.image-slide-container');
                if (nextImage.length > 0) {
                    container.find('.image-slide-container').removeClass('active');
                    nextImage.addClass('active');
                    if (container.closest('.modal-dialog').is(":visible")) {
                        container.find('.large-image').attr('src', nextImage.find('img').attr('data-zoom'));
                    } else {
                        $('#prd-image').attr('src', nextImage.find('img').attr('data-image'));
                        $('#prd-image').attr('src', nextImage.find('data-zoom').attr('data-zoom'));
                    }
                }

                if (container.closest('.image-carousel').length > 0) {
                    var offsetEl = nextImage.prop('offsetLeft');
                    var widthEl = nextImage.prop('offsetWidth');
                    var offsetP = nextImage.parent().prop('offsetLeft');
                    var widthP = nextImage.parent().prop('offsetWidth');
                    if (offsetEl + widthEl > offsetP + widthP) {
                        var scroller = 83;
                        var right = parseInt(nextImage.parent().css('right')) + scroller;
                        if (right >= 0) {
                            nextImage.parent().css('right', right + 'px');
                        }
                    }
                }
            });
