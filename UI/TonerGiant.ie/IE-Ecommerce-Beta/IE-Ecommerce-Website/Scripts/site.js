...
function signinComplete(data) {
    _gaq.push(['_trackEvent', 'Login', data.responseJSON.issuccess ? 'Successful' : 'UnSuccessful']);
...
function identComplete(data) {
    ...
    _gaq.push(['_trackEvent', 'Login', response.savereturn.IsSuccess ? 'Successful' : 'UnSuccessful']);
...
//function pcaCall(text, type, id, elem) {
//    var xtype = 'Find';
//    if (type !== '') {
//        if (type === 'Address') {
//            xtype = 'Retrieve';
...
//        async: false,
//        success: function (data) {
//            var add = elem.closest('.add');
//            if (data.savereturn.IsSuccess) {
//                if (xtype === "Retrieve") {
//                    var oJson = JSON.parse(data.savereturn.Message);
//                    if (oJson.length > 0) {
//                        var addFields = add.find('.add_fields');
//                        addFields.find('.add_line1').val(oJson[0].Company);
//                        addFields.find('.add_line2').val(oJson[0].Line1);
//                        addFields.find('.add_line3').val(oJson[0].Line2);
//                        addFields.find('.add_line4').val(oJson[0].City);
//                        addFields.find('.add_line5').val(oJson[0].Province);
//                        addFields.find('.add_postcode').val(oJson[0].PostalCode);
//                        addFields.removeClass('g-d-n');

//                        add.find('.add_pca').addClass('g-d-n');
//                        add.find('.add_manualentry').addClass('g-d-n');
//                        add.find('.add_search').val('');
//                        add.find('.add_pcalist').empty();
//                    }
//                } else {
//                    if (data.savereturn.Html !== '') {
//                        add.find('.add_pcalist').html(data.savereturn.Html);
//                    } else {
//                        add.find('.add_pcalist').html('no results found');
//                    }
//                    add.find('.add_pca').removeClass('g-d-n');
//                    add.find('.add_pcalist').width(add.find('.add_search').prop('clientWidth'));
//                }
//            } else {
//                add.find('.add_pcalist').html(data.savereturn.Html);
//                add.find('.add_pca').removeClass('g-d-n');
...
//        },
//        error: function (xhr, textStatus, thrownError) {
//            logAjaxScriptError("/Misc/PcaCall/", xhr, textStatus, thrownError);
//        }
//    });
//}
...
//$.fn.isInViewport = function (el, container) {
//    container = container || document.body;
//    const { top, bottom, height } = el.getBoundingClientRect();
//    const containerRect = container.getBoundingClientRect();

//    return top + 1 > containerRect.top && bottom - 1 < containerRect.bottom;
//};
...
$(function () {
    ...
    //    '.add_manualentry',
    //    function () {
    //        $(this).parent().find('.add_fields').removeClass('g-d-n');
    //        $(this).addClass('g-d-n');
    //    });
    //// The following statement prevents the onblur event from firing
    ...
    //    '.add_search',
    //    function () {
    //        $(this).closest('.add').find('.add_pca').addClass('g-d-n');
    //    });
    //$(document).on('keyup',
    //    '.add_search',
    //    function (e) {
    ...
    //        clearTimeout(autoCompTypingTimer);
    //        if (e.which === 13) {
    //            var selected = $(this).closest('.add').find('.add_pcaselected');
    //            var id = selected.attr('data-id');
    //            var type = selected.attr('data-type');
    //            var text = selected.attr('data-keyword');
    //            pcaCall(text, type, id, selected);
    //            event.stopPropagation();
    //        }
    //        var liSelected;
    //        if (e.which === 40) { // arrow down
    //            liSelected = $(this).closest('.add').find('.add_pcaselected');
    //            var next = liSelected.next();
    //            if (next.length > 0) {
    //                liSelected.removeClass('add_pcaselected');
    //                liSelected = next.addClass('add_pcaselected');
    //                if (!liSelected.isInViewport(liSelected.get(0), liSelected.parent().get(0))) {
    //                    liSelected.parent().scrollTop(liSelected.parent().scrollTop() + liSelected.outerHeight() - 0);
    //                }
    //            }
    ...
    //            if (prev.length > 0) {
    //                liSelected.removeClass('add_pcaselected');
    //                liSelected = prev.addClass('add_pcaselected');
    //                if (!liSelected.isInViewport(liSelected.get(0), liSelected.parent().get(0))) {
    //                    liSelected.parent().scrollTop(liSelected.parent().scrollTop() - liSelected.outerHeight() + 0);
    //                }
    //            }
    ...
    //        var text = self.attr('data-keyword');
    //        pcaCall(text, type, id, self);
    //    });
    //$(document).on('mouseenter', '.add_pcaitem', function () {
    //    var addpca = $(this).closest('.add_pca');
    //    addpca.find('.add_pcaitem').removeClass('add_pcaselected');
    //    $(this).addClass('add_pcaselected');
    //});
    ...
    // Page Specific
    if (isCurrentPage('misc/accountapplication')) {
        loadPca(window, document, "pca", "//NETGI11112.pcapredict.com/js/sensor.js");
