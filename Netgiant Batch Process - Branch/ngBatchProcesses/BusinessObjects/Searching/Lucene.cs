...
namespace ngBatchProcesses.BusinessObjects.Searching
{
    public class LuceneIndex
    {
        ...
        public void CreateLuceneIndexes(Dictionary<string, string> parms)
        {
            ...
            {
            SetupPrerequisites();
            stdFunc.AddToActivityLog("Started batch program with switch - createluceneindexes");

            foreach (var site in websites)
            {
                //Product
                var products = GetProducts(site.WebsiteID);
            ...
            stdFunc.AddToActivityLog("Finished batch program with switch - createluceneindexes");
            }
            catch (Exception ex)
            {
                errorOccurred = true;
                stdFunc.AddToActivityLog("**Error** - " + ex.Message);
            }
        ...
        {
            var filePath = stdFunc.LogActivity("createluceneindexes");
            if (errorOccurred)
                stdFunc.SendSimpleEmail("createluceneindexes", filePath);
        }
        ...
        private static void CreateProducts(List<product> products, IndexWriter writerProducts, Website site)
        {
            ...
            foreach (var prod in products)
            {
                ...
                //var productImage = ProductFunctions.GetProductImage(prod.partNo, site.WebsiteID, useHTTPS, prod.manufacturer.manufacturerName);
                var productImage = ProductFunctions.GetProductImage(prod.partNo, site.WebsiteID, prod.AxisFields.supressOpenRangeImage ?? false);

                if (!productImage.Contains("http"))
                    productImage = (useHTTPS ? "https://" : "http://") + site.WebURL + productImage;
        ...
        private List<product> GetProducts(int websiteFK)
        {
            using (ngmdEntities db = new ngmdEntities())
            {
                return db.product
                    ...
                    .Where(x => x.websiteInventory.Any(y => y.websiteFK == websiteFK) &&
        ...
        private List<categoryCode> GetCategories(int websiteFK)
        {
            ...
            using (ngmdEntities db = new ngmdEntities())
            {
                dbCatgeories = db.categoryCode
                    ...
                    .Where(x => x.websiteFK == websiteFK &&
                        !x.categoryCodeName.Contains("_OLD"))
                    .ToList();
            }
