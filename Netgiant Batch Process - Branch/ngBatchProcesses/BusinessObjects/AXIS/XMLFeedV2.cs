...
namespace ngBatchProcesses.BusinessObjects.Axis
{
    ...
    {
        private static StandardFunctions _std = new StandardFunctions();
        private static List<string> _fieldNames;
        private static bool _isFull;
        private static string _currentPartNo;
        private static bool _errorOccured;
        private static List<int> _axisQueueIds;
        private static int websiteCount;
        private static XNamespace af = "http://resources.axisfirst.co.uk/xml/";
        private static XNamespace wc = "http://www.w3.org/2001/XMLSchema-instance";
        ...
        public static void ProcessAxisQueue()
        {
            BuildXML();
        }
        ...
        private static void BuildXML()
        {
            try
            {
                _std.AddToActivityLog("Started with switch - processaxisqueuev2");
            ...
            {
                _std.AddToActivityLog(ex.Message);
            }

            LogActivity();
        }

        private static XDocument ConstructXML()
        {
        ...
        private static bool CheckValidity(DataRow row)
        {
            return ((int)row["productStatusFK"] == 1 || (int)row["productStatusFK"] == 9) && row["prices"].ToString() == "";
        }

        private static DataTable GetAxisQueue()
        {
            try
            {
                AXISQueueFeedServices axis = new AXISQueueFeedServices();
                _std.AddToActivityLog("Successfully extracted data from SQL" + Environment.NewLine);
                return axis.GetAXISQueueFeedData();
            }
            catch (Exception ex)
            {
                _errorOccured = true;
        ...
        private static void ExtractKeyData(DataRow row)
        {
            try
            {
                _currentPartNo = row["partNo"].ToString();
                _std.AddToActivityLog("Processing partNo - " + _currentPartNo);
                ...
                _std.AddToActivityLog("Successully extracted the key data for partNo - " + _currentPartNo);
            }
            catch (Exception ex)
            {
                _errorOccured = true;
                _std.AddToActivityLog("Error extracting the key data for Axis Queue record");
                _std.AddToActivityLog(ex.Message);
            }
        }
        ...
        private static XElement BuildProductElement(DataRow row)
        {
            try
            {
        ...
        private static void BuildProductDetailElement(XElement xml, DataRow row)
        {
            try
            {
        ...
        private static void AddProductAlias(XElement productDetail, DataRow row)
        {
            try
            {
        ...
        private static void AddProductNotes(XElement productDetail, DataRow row)
        {
            try
            {
        ...
        private static void AddResaleable(XElement productDetail, DataRow row)
        {
            try
            {
        ...
        private static void AddDiscontinued(XElement productDetail, DataRow row)
        {
            try
            {
        ...
        private static string AddProductName(string name, XElement xml, DataRow row)
        {
            var leftover = "";
        ...
        private static XElement AddElement(string nodeName, string fieldName,
        ...
        private static XElement AddAttribute(string attrName, string attrValue, XElement element)
        {
            if (element != null)
        ...
        private static void BuildAttributesElement(XElement prodElement, DataRow row)
        {
            try
            {
        ...
        private static void BuildPricingElement(XElement prodElement, DataRow row)
        {
            try
            {
        ...
        private static void AddStockPricingElements(string[] tgPriceDetails, XElement priceElement)
        {
        ...
        private static string ExtractDefaultPrice(string[] sitesPricing)
        {
        ...
        private static void AddSiteSpecificPricingElements(XElement matrix, int i, string currentSite, string[] breakQuantities)
        {
        ...
        private static void BuildEbusinessElement(XElement prodElement, DataRow row)
        {
            try
            {
                if (PassEbusinessSection())
                {
                    ...
                    foreach (int siteId in websiteIds)
                    {
                        ...
                        //Suppress CNET Image
                        //string supCnetImage = string.Empty;
                        //if (row["supressCnetImage"].ToString() == "True")
                        //{
                        //    supCnetImage = "true";
                        //}
                        //else
                        ...
                        //    supCnetImage = "false";
                        //}
                        //siteElement.Add(new XElement(af + "SuppressCNetImage", supCnetImage));

                        //Suppress CNET Desc
                        //string supCnetDesc = string.Empty;
                        //if (row["supressCnetDesc"].ToString() == "True")
                        //{
                        //    supCnetDesc = "true";
                        //}
                        //else
                        ...
                        //    supCnetDesc = "false";
                        //}
                        //siteElement.Add(new XElement(af + "SuppressCNetDescription", supCnetDesc));
        ...
        private static void BuildFeedElement(DataRow row, XElement siteElement)
        {
        ...
        private static void GetFeedElement(XElement siteElement, string feedName, string include)
        {
        ...
        private static bool PassEbusinessSection()
        {
            ...
            //if (PassField("supressCnetDesc")) { return true; }
            //if (PassField("supressCnetImage")) { return true; }
        ...
        private static void BuildAdditionalEbusiness(string[] eBusinessArray, XElement siteElement)
        {
        ...
        private static void BuildWebSpecific(DataRow row, XElement siteElement)
        {
        ...
        private static void BuildSuppliersElement(XElement prodElement, DataRow row)
        {
            try
            {
        ...
        private static void BuildComponentsElement(XElement prodElement, DataRow row)
        {
            try
            {
        ...
        private static void AddProductToXML(XDocument xml, XElement prodElement)
        {
            var query = xml.Descendants(af + "Products").First();
            query.Add(prodElement);

            _std.AddToActivityLog("Successfully added the product element to the main xml." + Environment.NewLine);
        }

        private static bool PassField(string fieldName)
        {
        ...
        {
            string activityPath = _std.LogActivity("processaxisqueuev2");
            if (_errorOccured && Properties.Settings.Default.Environment == "Live")
            {
                _std.SendSimpleEmail("processaxisqueuev2", activityPath);
            }
        }
        ...
        private static void UpdateCompletedDate()
        {
            try
            {
                ...
                _std.AddToActivityLog("Successfully updated completed dates");
            }
            catch (Exception ex)
            {
        ...
        private static void ClearAxisQueueRecords()
        {
            try
            {
                ...
                _std.AddToActivityLog("Successfully cleared Axis Queue Records");
            }
            catch (Exception ex)
            {
        ...
        private static void GetWebsiteCount()
        {
            using (ngmdEntities db = new ngmdEntities())
            {
