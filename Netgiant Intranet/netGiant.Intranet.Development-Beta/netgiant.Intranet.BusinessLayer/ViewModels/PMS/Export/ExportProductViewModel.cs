...
{
    public class ExportProductViewModel : CommonViewModel
    {
        ...
        private void InsertAxisFieldsCSVData(product prod, CsvRow newRow)
        {
            if (PostedAxisFields != null)
            {
                ...
                AddCsvData(newRow, "reSaleable", af != null ? af.reSaleable == true ? "Y" : "N" : "", ExportAxisFieldsFieldDictionary, PostedAxisFields);
                ...
                AddCsvData(newRow, "discontinuedItem", af != null ? af.discontinuedItem == true ? "Y" : "N" : "", ExportAxisFieldsFieldDictionary, PostedAxisFields);
        ...
        private void InsertAxisFieldsAdditionalData(AxisFields af, CsvRow newRow)
        {
            ...
            if (afa != null)
            {
                ...
                var priorityNote = "";
                if (afa.priorityNote != null)
                    priorityNote = afa.priorityNote.Replace(Environment.NewLine, "");
                AddCsvData(newRow, "priorityNote", priorityNote, ExportAxisFieldsFieldDictionary, PostedAxisFields);
                ...
                AddCsvData(newRow, "googleFeedInclude", afa.googleFeedInclude != null ? afa.googleFeedInclude == true ? "Y" : "N" : "", ExportAxisFieldsFieldDictionary, PostedAxisFields);
                ...
                AddCsvData(newRow, "bespokeFeedInclude", afa.bespokeFeedInclude != null ? afa.bespokeFeedInclude == true ? "Y" : "N" : "", ExportAxisFieldsFieldDictionary, PostedAxisFields);
                AddCsvData(newRow, "bespokeFeedSite", afa.bespokeFeedSite, ExportAxisFieldsFieldDictionary, PostedAxisFields);
                AddCsvData(newRow, "bespokeFeedUseCustomShipCost", afa.bespokeFeedUseCustomShipCost != null ? afa.bespokeFeedUseCustomShipCost == true ? "Y" : "N" : "", ExportAxisFieldsFieldDictionary, PostedAxisFields);
        ...
        private void InsertProductCSVData(product prod, CsvRow newRow)
        {
            if (PostedProductFields != null)
            {
                ...
                string productStatus = DataCache.GetNgmdLookups(x => x.LookupType.LookupTypeName == "ProductStatus" && x.AltLookupId == prod.productStatusFK)
                       .FirstOrDefault()?.LookupName;
                AddCsvData(newRow, "productStatusFK", productStatus ?? "", ExportProductFieldDictionary, PostedProductFields);
                ...
                AddCsvData(newRow, "capacity", prod.capacity, ExportProductFieldDictionary, PostedProductFields);

                AddCsvData(newRow, "supplierLastUpdate", prod.supplierLastUpdate, ExportProductFieldDictionary, PostedProductFields);

                AddCsvData(newRow, "secondaryCrossSellGroup", prod.secondaryCrossSellGroupIdent, ExportProductFieldDictionary, PostedProductFields);

                if (ExportProductFieldDictionary.ContainsKey("secondaryCategories") &&
                                                PostedProductFields.Contains("secondaryCategories"))
                {
                    if (SelectedWebsiteFK > 0)
                    {
                        websiteInventory wi = prod.websiteInventory.Where(x => x.websiteFK == SelectedWebsiteFK).FirstOrDefault();

                        if (wi != null && wi.secondaryCategoryLookup != null)
                        {
                            string secondaryCategories = "";
                            foreach (var secondCat in wi.secondaryCategoryLookup)
                            {
                                secondaryCategories += secondCat.categoryCodeFK.ToSafeString() + ", ";
                            }
                            secondaryCategories = secondaryCategories.TrimEnd(',', ' ');
                            AddCsvData(newRow, "secondaryCategories", secondaryCategories, ExportProductFieldDictionary, PostedProductFields);
                        }
                        else
                        {
                            AddBlankCell(newRow);
                ...
                if (ExportProductFieldDictionary.ContainsKey("assemblyComponents") &&
                                                PostedProductFields.Contains("assemblyComponents"))
                {
                    if (prod.assemblyComponent != null)
                    {
                        string assemblyComponents = "";
                        foreach (var cmpt in prod.assemblyComponent)
                        {
                            assemblyComponents += cmpt.product1.partNo + ", ";
                        }
                        assemblyComponents = assemblyComponents.TrimEnd(',', ' ');
                        AddCsvData(newRow, "assemblyComponents", assemblyComponents, ExportProductFieldDictionary, PostedProductFields);
                    }
                    else
                    {
                        AddBlankCell(newRow);
                ...
                if (ExportProductFieldDictionary.ContainsKey("primaryebusinessGroup") &&
                                                PostedProductFields.Contains("primaryebusinessGroup"))
                {
                    if (prod.AxisEbusinessMapping != null)
                    {
                        string primaryeBusinessGroup = "";
                        foreach (var ebusmp in prod.AxisEbusinessMapping)
                        {
                            if (ebusmp.eBusinessPrimary)
                            {
                                primaryeBusinessGroup = ebusmp.AxisEbusiness.eBusinessCode;
                                break;
                            }
                        }
                        AddCsvData(newRow, "primaryebusinessGroup", primaryeBusinessGroup, ExportProductFieldDictionary, PostedProductFields);
                    }
                    else
                    {
                        AddBlankCell(newRow);
                ...
                if (ExportProductFieldDictionary.ContainsKey("secondaryebusinessGroups") &&
                                                PostedProductFields.Contains("secondaryebusinessGroups"))
                {
                    if (prod.AxisEbusinessMapping != null)
                    {
                        string secondaryeBusinessGroups = "";
                        foreach (var ebusmp in prod.AxisEbusinessMapping)
                        {
                            if (!ebusmp.eBusinessPrimary)
                                secondaryeBusinessGroups += ebusmp.AxisEbusiness.eBusinessCode + ", ";
                        }
                        secondaryeBusinessGroups = secondaryeBusinessGroups.TrimEnd(',', ' ');
                        AddCsvData(newRow, "secondaryebusinessGroups", secondaryeBusinessGroups, ExportProductFieldDictionary, PostedProductFields);
                    }
                    else
                    {
                        AddBlankCell(newRow);
                ...
                AddCsvData(newRow, "dateCreated", prod.dateCreated, ExportProductFieldDictionary, PostedProductFields);
                AddCsvData(newRow, "dateLastInStock", prod.dateLastInStock, ExportProductFieldDictionary, PostedProductFields);
            }
        }
        ...
        private void SetAxisFieldsColumnHeadings(CsvRow firstRow)
        {
            if (PostedAxisFields != null)
            {
                ...
                AddCsvColumn(firstRow, "priorityNote", ExportAxisFieldsFieldDictionary, PostedAxisFields);
        ...
        private void SetProductColumnHeadings(CsvRow firstRow)
        {
            if (PostedProductFields != null)
            {
                ...
                AddCsvColumn(firstRow, "capacity", ExportProductFieldDictionary, PostedProductFields);
                AddCsvColumn(firstRow, "supplierLastUpdate", ExportProductFieldDictionary, PostedProductFields);
                AddCsvColumn(firstRow, "secondaryCategories", ExportProductFieldDictionary, PostedProductFields);
                AddCsvColumn(firstRow, "assemblyComponents", ExportProductFieldDictionary, PostedProductFields);
                AddCsvColumn(firstRow, "primaryebusinessGroup", ExportProductFieldDictionary, PostedProductFields);
                AddCsvColumn(firstRow, "secondaryebusinessGroups", ExportProductFieldDictionary, PostedProductFields);
                AddCsvColumn(firstRow, "secondaryCrossSellGroup", ExportProductFieldDictionary, PostedProductFields);
                AddCsvColumn(firstRow, "dateCreated", ExportProductFieldDictionary, PostedProductFields);
                AddCsvColumn(firstRow, "dateLastInStock", ExportProductFieldDictionary, PostedProductFields);
            }
        }
        ...
        private List<product> GetProducts()
        {
            ...
            using (ngmdEntities db = new ngmdEntities())
            {
                ...
                if (PostedProductFields.Contains("secondaryCategories") && SelectedWebsiteFK > 0)
                    query = query.Include("websiteInventory.secondaryCategoryLookup");

                if (PostedProductFields.Contains("assemblyComponents"))
                {
                    query = query.Include("assemblyComponent");
                    query = query.Include("assemblyComponent.product");
                    query = query.Include("assemblyComponent.product1");
                }

                if (PostedProductFields.Contains("primaryebusinessGroup") || PostedProductFields.Contains("secondaryebusinessGroups"))
                    query = query.Include("AxisEbusinessMapping.AxisEbusiness");
