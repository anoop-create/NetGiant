    ...
    public class ProductViewModel : HelperViewModel
    {
        ...
        public ProductViewModel Get(int? block, string search, string searchBy, string orderBy)
        {
            ...
            try
            {
                ...
                {
                    IQueryable<product> list = db.product.Include(p => p.productGroup).Include(p => p.productStatus).Include(p => p.salesAreaGroup)
                        .Include(p => p.manufacturer).Include(p => p.websiteInventory).Include("websiteInventory.productPrice")
                    ...
                    if (!string.IsNullOrEmpty(search))
                    {
                        ...
                        switch (searchBy)
                        {
                            ...
                            case "productGroup":
                                ...
                                break;
                            case "productStatus":
                                list = list.Where(x => x.productStatus.productStatusName.ToLower().Contains(search));
                                break;
                            case "salesAreaGroup":
                    ...
                    switch (orderBy)
                    {
                        ...
                        case "manufacturerDesc":
                            ...
                            break;
                        case "productStatusAsc":
                            list = list.OrderBy(x => x.productStatus.productStatusName);
                            break;
                        case "productStatusDesc":
                            list = list.OrderByDescending(x => x.productStatus.productStatusName);
                            break;
        ...
        public static ProductViewModel Create(int id)
        {
            ...
            try
            {
                using (ngmdEntities db = new ngmdEntities())
                {
                    ...
                    {
                        model.prod = db.product.Include("websiteInventory.categoryCode")
                        ...
                        {
                            model.SelectedWebsites.Add(db.Websites.FirstOrDefault(x => x.WebsiteID == webInventory.websiteFK));
                        }
                    ...
                    model.AllWebsites = db.Websites.ToList().AsQueryable();
                    ...
                    model.AllCategoryCodes = SelectListViewModel.AllCategoryCodes();
                    model.AllProductItemType = SelectListViewModel.GetAllProductItemType();
        ...
        private void UpdateWebsiteInventory(bool isNew)
        {
            using (ngmdEntities db = new ngmdEntities())
            {
                ...
                if (null != SelectedWebsiteIDs)
                {
                    ...
                    int[] nonSelectedWebsiteIDs = db.Websites.Select(x => x.WebsiteID).Except(SelectedWebsiteIDs).ToArray();
        ...
        private static void CompareAndCreateQueueEntry(object a, object b, string entityName,
                                            string fieldName, int axisQueueID)
        {
            if (a == null)
                a = string.Empty;
            if (b == null)
        ...
        //                af.dateLastUpdate = DateTime.Now;

        //                if (af.productFK == 0)
        //                {
        //                    af.productFK = prod.productID;
        ...
        //                foreach (AxisFieldsAdditional afa in af.AxisFieldsAdditional)
        //                {
        //                    if (afa.productFK == 0)
        //                    {
        //                        afa.dateLastUpdate = DateTime.Now;
        //                        afa.productFK = prod.productID;
        //                        db.Entry(afa).State = EntityState.Added;
        //                    }
        //                    else
        ...
        //                //db.SaveChanges();
        //                CreateProductAXISQueue(false, prod);
        ...
        //                db.product.Add(prod);
        //                db.SaveChanges();
        //                CreateProductAXISQueue(true, prod);
        //            }
        ...
        //                        CreateWebInventoryAXISQueue(webInventory, GetEnumDescription(CRUD.Create), "productFK");
        //                    }
        //                    else
        //                    {
        //                        websiteInventory wi = db.websiteInventory.Where(x => x.productFK == prod.productID && x.websiteFK == websiteID).First();
        ...
        //public static void CreateProductAXISQueue(bool isNew, product newProduct)
        //{
        //    //string[] ignoreList = { "productID", "dateLastUpdate" };

        //    using (ngmdEntities db = new ngmdEntities())
        //    {
        //        //Create AXIS Queue for the product if doesn't exist
        //        AXISQueue axisQueue = null;
        //        if (!db.AXISQueue.Any(x => x.productFK == newProduct.productID))
        //        {
        //            axisQueue = new AXISQueue() { productFK = newProduct.productID, dateLastUpdated = DateTime.Now };
        //            db.AXISQueue.Add(axisQueue);
        //            db.SaveChanges();
        //        }
        //        else
        //        {
        //            axisQueue = db.AXISQueue.FirstOrDefault(x => x.productFK == newProduct.productID);
        //        }
        ...
        //            product existingProduct = db.product.Find(newProduct.productID);

        //            //If changing from a 'No Status' to 'Active'
        //            if (existingProduct.productStatusFK == 4 && newProduct.productStatusFK == 1)
        //            {
        //                CreateQueueDetailsEntry("All", "All", axisQueue.AXISQueueID, CRUD.Create);
        //            }
        //            else
        //            {
        //                CompareProduct(newProduct, existingProduct, axisQueue);
        //                CompareAxisFields(newProduct, existingProduct, axisQueue);
        //                CompareAxisFieldsAdditional(newProduct, existingProduct, axisQueue);
        //            }

        //            //product existingProduct = db.product.Find(newProduct.productID);
        //            //Type entityProductType = newProduct.GetType();

        //            //foreach (PropertyInfo propertyInfo in entityProductType.GetProperties(BindingFlags.Public | BindingFlags.Instance)
        ...
        //            //        try
        //            //        {
        //            //            valueA = propertyInfo.GetValue(newProduct, null);
        //            //            valueB = propertyInfo.GetValue(existingProduct, null);
        //            //        }
        //            //        catch (Exception ex)
        //            //        {
        //            //            throw new ApplicationException("Reflection Error with field - " +
