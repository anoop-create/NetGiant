...
using static DP001BusinessLogic.ViewModels.SharedViewModel;

namespace DP001Website.Models
{

    //public enum ContractPermissionType
    //{
    //    SalesHistory,
    //    CloneChannel,
    //    PriceRuleBanding,
    ...
    //public enum ContractPermissonCheck
    //{
    //    ContractOnly,
    //    ContractAndTenant
    //}

    public enum TenantPermissonCheck
    {
        IsFeatureOn,
        IsFeatureAllowed
    }
    ...
    {
        public static bool TenantPermisson(
            this HtmlHelper htmlHelper,
            string fieldName,
            TenantPermissonCheck permissionCheck)
        {
            var cm = new CommonModel();
            return cm.TenantPermissionCheck(fieldName, permissionCheck);
        }
    }
    ...
    {
        public static MvcHtmlString DropDownListExtended(
            this HtmlHelper htmlHelper,
            string id,
            List<SelectListItemExtended> data,
            object htmlAttributes,
            string selectedValue,
            string defaultText = null)
        {
            var select = new TagBuilder("select");
            var selectAttributes = HtmlHelper.AnonymousObjectToHtmlAttributes(htmlAttributes);
            select.Attributes.Add("id", id);

            foreach (var att in selectAttributes)
            {
                select.Attributes[att.Key] = att.Value.ToString();
            }

            if (defaultText != null)
                data.Insert(0, new SelectListItemExtended { Text = defaultText });
            ...
            foreach (var item in data)
            {
                ...
                {
                    if (att.Value != null)
                        option.Attributes[att.Key] = att.Value.ToString();
                }
