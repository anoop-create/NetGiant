...
namespace DP001Website.Models
{
    public class CommonModel
    {
        ...
        public bool TenantPermissionCheck(string fieldName, TenantPermissonCheck permissionCheck)
        {
            var currentUserTenant = GetTenant();
            var allowFieldName = "Allow" + fieldName;

            switch (permissionCheck)
            {
                case TenantPermissonCheck.IsFeatureOn:

                    var featureNameProperty = typeof(TenantSetting).GetProperty(fieldName);
                    var allowFeatureNameProperty = typeof(TenantSetting).GetProperty(allowFieldName);

                    return allowFeatureNameProperty != null && featureNameProperty != null && (bool)featureNameProperty.GetValue(currentUserTenant, null) && (bool)allowFeatureNameProperty.GetValue(currentUserTenant, null);

                case TenantPermissonCheck.IsFeatureAllowed:

                    var property = typeof(TenantSetting).GetProperty(allowFieldName);
                    return property != null && (bool)property.GetValue(currentUserTenant, null);
