...
GridMvc = (function ($) {
    ...
    */
    gridMvc.prototype.applyFilterValues = function (initialUrl, columnName, values, skip) {
        ...
        var filters = this.jqContainer.find(".tableFloatingHeaderOriginal .grid-filter");
    ...
    gridMvc.prototype.getFilterQueryData = function (columnName, values) {
        ...
        for (var i = 0; i < values.length; i++) {
            if (columnName == "Pi.PriceRule.RuleName" && values[i].filterValue == "Null") {
                url += "rn=no";
            } else {
                url += "grid-filter=" +
                    ...
                    values[i].filterType +
                    "__" +
                    encodeURIComponent(values[i].filterValue);
            }
...
TextFilterWidget = (function ($) {
    ...
    */
    ...
    //### The following function has been customised for Priceology ###
