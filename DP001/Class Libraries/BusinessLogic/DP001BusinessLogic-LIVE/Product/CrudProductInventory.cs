...
namespace DP001BusinessLogic
{
    public class CrudProductInventory
    {
        ...
        public bool Create(List<ProductInventory> products, Channel channel)
        {
            ...
            using (DP001Entities db = new DP001Entities())
            {
                ...
                foreach (ProductInventory pi in products)
                {
                    dt.Rows.Add(0, pi.ChannelFK, pi.BrandFK, pi.ManufacturerPartNo.Truncate(45), pi.Description.Truncate(200), pi.ClientProductID.Truncate(45), pi.LnkdBrandFK, 
                        ...
                        pi.IsKeyLine, pi.CustomProductField1, pi.CustomProductField2, pi.CustomProductField3, pi.CustomProductField4, pi.CustomProductField5, 
                        pi.CustomProductField6, pi.CustomProductField7, pi.CustomProductField8, pi.CustomProductField9, pi.CustomProductField10, pi.VariantOf.Truncate(50));
                }
            }
            ...
            CommonDataFunctions.CreateLogEntry(channel, "END CreateProductData - COMPUTE", "Information");
            CommonDataFunctions.CreateLogEntry(channel, "START CreateProductData - SQL", "Information");

            var isSuccess = SQL.ExecuteStoredProcedure("DP001", "CreateUpdateProductInventory", sqlParms, channel.ChannelID);

            if (!isSuccess)
            {
                CommonDataFunctions.CreateLogEntry(channel, "Unable to load products due to errors found. Please contact support.", "Notification");
            }

            CommonDataFunctions.CreateLogEntry(channel, "END CreateProductData - SQL", "Information");
        ...
        public ProductInventory Read(string clientProductId, int channelId)
        {
            using (DP001Entities db = new DP001Entities())
            {
                return db.ProductInventories
                    .Where(x => x.ClientProductID == clientProductId && x.ChannelFK == channelId)
                    .FirstOrDefault();
            }
        }
        ...
        public int ReadCount(int channelId)
        {
            ...
            {
                NoLockInterceptor.ApplyNoLock = true;
                return db.ProductInventories.Where(x => x.ChannelFK == channelId && x.Lookup1.LookupName == "Active").Count();
            }
        }
        ...
        public int ReadSupplierExceptionCount(int channelId, int matchLimit)
        {
            using (DP001Entities db = new DP001Entities())
            {
                ...
                return db.ProductInventories.Where(x => x.ChannelFK == channelId && 
                    x.SupplierInventories.Count <= matchLimit && x.Lookup1.LookupName == "Active").ToList().Count;
            }
        }
        ...
        public int ReadCompetitorExceptionCount(int channelId, int matchLimit)
        {
            using (DP001Entities db = new DP001Entities())
            {
                NoLockInterceptor.ApplyNoLock = true;
                return db.ProductInventories.Where(x => x.ChannelFK == channelId &&
                    x.CompetitorInventories.Count <= matchLimit && x.Lookup1.LookupName == "Active").ToList().Count;
            }
        }
