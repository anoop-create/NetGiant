...
namespace DP001BusinessLogic
{
    public class CrudSupplierInventory
    {
        ...
        public bool Create(List<SupplierInventory> suppliers, Channel channel)
        {
            ...
            CommonDataFunctions.CreateLogEntry(channel, "END CreateSupplierData - COMPUTE", "Information");
            CommonDataFunctions.CreateLogEntry(channel, "START CreateSupplierData - SQL", "Information");

            var isSuccess = SQL.ExecuteStoredProcedure("DP001", "CreateUpdateSupplierInventory", sqlParms, channel.ChannelID);

            CommonDataFunctions.CreateLogEntry(channel, "END CreateSupplierData - SQL", "Information");

            if (!isSuccess)
            {
                CommonDataFunctions.CreateLogEntry(channel, "Unable to load suppliers due to errors found. Please contact support.", "Notification");
            }
