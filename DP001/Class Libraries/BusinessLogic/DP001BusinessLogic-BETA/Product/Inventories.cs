    ...
    public class Inventories
    {
        ...
        private static bool FeedHasData(DownloadedFileData feed, Channel channel)
        {
            ...
            if (feed.FileType == FtpFileType.SkuuudleSummary)
            {
                ...
                using (TextFieldParser csvReader = new TextFieldParser(new StringReader(fileContent)))
                {
                    ...
                    var competitorNameColumn = CommonFunctions.LookupFieldIndex(feed, headings, "Competitor name", indexList);
                    var averageRatingColumn = CommonFunctions.LookupFieldIndex(feed, headings, "Average rating", indexList);
                    var numberOfReviewsColumn = CommonFunctions.LookupFieldIndex(feed, headings, "No. of reviews (qty)", indexList);
        ...
        private void InitializeTenant()
        {
            try
            {
                ...
                _processStartTime = CommonDataFunctions.GetCurrentDateTime(true);
        ...
        private static List<SupplierInventory> GetSupplierList(Channel channel, DownloadedFileData file, Supplier supp)
        {
            ...
            using (var csvReader = new TextFieldParser(new StringReader(fileContent)))
            {
                ...
                var indexList = new List<int>();
                var manufacturerPartNoColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.ManufacturerPartNo, indexList);
                var manufacturerColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.Brand, indexList, false);
                var stockColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.StockQuantity, indexList);
                var priceColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.Price, indexList);
                var descriptionColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.Description, indexList);
                var clientProductIdColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.ClientProductID, indexList, false);
                    ...
                    while (!csvReader.EndOfData)
                    {
                        try
                        {
                            ...
                            var supplier = new SupplierInventory()
                            {
                                ...
                                BrandName = brand,
                                StockQuantity = !string.IsNullOrEmpty(stock) ? (int)Math.Floor(Convert.ToDecimal(stock)) : 0,
                                Price = !string.IsNullOrEmpty(price) ? Math.Round(Convert.ToDecimal(price), 2) : 0,
        ...
        {
            // Competitor Brand
            _competitorInvList = _competitorInvList.Where(x => !_providerExclusions.Any(
                    y => y.BrandName == x.BrandName &&
                         y.Lookup.LookupName == "Competitor Inventory" &&
                         y.Lookup1.LookupName == "Brand" &&
                         y.ProviderFK == x.CompetitorFK))
                .ToList();

            if (channel.UseClientProductId)
            {
                _competitorInvList = _competitorInvList.Where(x => !_providerExclusions.Any(y => y.ClientProductID == x.ClientProductID &&
                            y.Lookup.LookupName == "Competitor Inventory" &&
                            y.Lookup1.LookupName == "Item" &&
                            y.ProviderFK == x.CompetitorFK)).ToList();
            }
            else
            {
                _competitorInvList = _competitorInvList.Where(x => !_providerExclusions.Any(y => y.BrandName == x.BrandName
        ...
        private static List<SupplierInventory> RemoveSupplierExclusions(Channel channel)
        {
            ...
            _competitorInvList = _competitorInvList.Where(x => !_providerExclusions.Any(y => y.BrandName == x.BrandName &&
                            ...
                            y.Lookup1.LookupName == "Provider Brand" &&
                            y.ProviderFK == x.CompetitorFK)).ToList();
        ...
        private static List<ProductInventory> GetProductList(Channel channel, DownloadedFileData file)
        {
            ...
            using (TextFieldParser csvReader = new TextFieldParser(new StringReader(fileContent)))
            {
                ...
                var indexList = new List<int>();
                var manufacturerPartNoColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.ManufacturerPartNo, indexList);
                var manufacturerColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.Brand, indexList);
                var descriptionColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.Description, indexList);
                var clientProductIDColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.ClientProductID, indexList, false);
                var lnkdManufacturerColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.LnKdManufacturer, indexList, false);
                var lnkdManufacturerPartNoColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.LnKdManufacturerPartNo, indexList, false);
                var productCategoryColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.ProductCategory, indexList);
                var isKeyLineColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.IsKeyLine, indexList, false);
                var customField1Column = CommonFunctions.LookupFieldIndex(file, firstLine, fields.CustomField1, indexList, IsCustomFieldRequired(1));
                var customField2Column = CommonFunctions.LookupFieldIndex(file, firstLine, fields.CustomField2, indexList, IsCustomFieldRequired(2));
                var customField3Column = CommonFunctions.LookupFieldIndex(file, firstLine, fields.CustomField3, indexList, IsCustomFieldRequired(3));
                var customField4Column = CommonFunctions.LookupFieldIndex(file, firstLine, fields.CustomField4, indexList, IsCustomFieldRequired(4));
                var customField5Column = CommonFunctions.LookupFieldIndex(file, firstLine, fields.CustomField5, indexList, IsCustomFieldRequired(5));
                var customField6Column = CommonFunctions.LookupFieldIndex(file, firstLine, fields.CustomField6, indexList, IsCustomFieldRequired(6));
                var customField7Column = CommonFunctions.LookupFieldIndex(file, firstLine, fields.CustomField7, indexList, IsCustomFieldRequired(7));
                var customField8Column = CommonFunctions.LookupFieldIndex(file, firstLine, fields.CustomField8, indexList, IsCustomFieldRequired(8));
                var customField9Column = CommonFunctions.LookupFieldIndex(file, firstLine, fields.CustomField9, indexList, IsCustomFieldRequired(9));
                var customField10Column = CommonFunctions.LookupFieldIndex(file, firstLine, fields.CustomField10, indexList, IsCustomFieldRequired(10));
                var variantOfColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.VariantOf, indexList, false);
                ...
                if (CommonFunctions.CheckValidColumns(indexList))
                {
                    ...
                    while (!csvReader.EndOfData)
                    {
                        if (currentRow > channel.TenantSetting.Contract.InventoryCount)
                        {
                            CommonDataFunctions.CreateLogEntry(channel, "Warning: Product feed contains more products than your current subscription allows. Please contact support to upgrade.", "Subscription");
                            break;
                        }
            ...
            HashSet<Competitor> activeCompetitors = null)
        {
            ...
            using (TextFieldParser csvReader = new TextFieldParser(new StringReader(fileContent)))
            {
                ...
                var manufacturerColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.Brand, indexList);
                var priceColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.Price, indexList);
                var competitorColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.Competitor, indexList);
                var clientProductIdColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.ClientProductID, indexList, false);
                var descriptionColumn = CommonFunctions.LookupFieldIndex(file, firstLine, fields.Description, indexList);
        ...
        private HashSet<Competitor> GetSkuudleLiteSummary(Channel _channel, DownloadedFileData file)
        {
            ...
            using (TextFieldParser csvReader = new TextFieldParser(new StringReader(fileContent)))
            {
                ...
                var indexList = new List<int>();
                var competitorNameColumn = CommonFunctions.LookupFieldIndex(file, headings, "Competitor name", indexList);
                var averageRatingColumn = CommonFunctions.LookupFieldIndex(file, headings, "Average rating", indexList);
                var numberOfReviewsColumn = CommonFunctions.LookupFieldIndex(file, headings, "No. of reviews (qty)", indexList);
                ...
                if (CommonFunctions.CheckValidColumns(indexList))
                {
                    while (!csvReader.EndOfData)
                    {
                        try
                        {
                            ...
                            var averageRating = GetRowFieldData(rowData, averageRatingColumn);
                            var numberOfReviews = GetRowFieldData(rowData, numberOfReviewsColumn);

                            decimal averageRatingDecimal;
                            int numberOfReviewsInt;
                            var averageRatingParsed = decimal.TryParse(averageRating, out averageRatingDecimal);
                            var numberOfReviewsParsed = int.TryParse(numberOfReviews, out numberOfReviewsInt);

                            var competitor = new Competitor()
                            {
                                ChannelFK = _channel.ChannelID,
                                CompetitorName = competitorName,
                                ReviewRating = !string.IsNullOrEmpty(averageRating) ? averageRatingParsed ? averageRatingDecimal : 0 : 0,
                                ReviewTotal = !string.IsNullOrEmpty(numberOfReviews) ? numberOfReviewsParsed ? numberOfReviewsInt : 0 : 0,
                                IsActive = SetSkuudleLiteActive(_channel,
                                    !string.IsNullOrEmpty(averageRating) ? averageRatingParsed ? averageRatingDecimal : 0 : 0,
                                    !string.IsNullOrEmpty(numberOfReviews) ? numberOfReviewsParsed ? numberOfReviewsInt : 0 : 0)
                            };
                        ...
                        {
                            CommonDataFunctions.CreateLogEntry(_channel, "**ERROR** in file: " + file.Settings.FTPFileName +
                                ": " + e.Message, "Error");
                            CommonDataFunctions.CreateLogEntry(_channel, "**ERROR**: " + e.StackTrace, "Error");
                        }
                    }
        ...
        private static void CreateProductData(Channel channel)
        {
            ...
            foreach (var product in _productList)
            {
                ...
                {
                    CommonDataFunctions.CreateLogEntry(channel, "Failed in CreateProductData for Product ID: " + product.ClientProductID + ". Error: " +
                        e.Message + " Stack: " + e.StackTrace, "Error");
                    CommonDataFunctions.CreateLogEntry(channel, "Error creating product data. Product ID: " + product.ClientProductID + ". Please contact support", "Notification", true);
        ...
        private static void CreateSupplierData(Channel channel)
        {
            ...
            try
            {
                ...
                if (_providerExclusions.Any(x => x.Lookup.LookupName == "Supplier Inventory"))
                {
        ...
        private void CreateCompetitorInventoryData(Channel channel)
        {
            ...
            try
            {
                ...
                if (_providerExclusions.Any(x => x.Lookup.LookupName == "Competitor Inventory"))
                {
