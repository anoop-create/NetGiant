...
namespace DP001BusinessLogic.ViewModels
{
    public class SharedViewModel
    {
        ...
        public static List<SelectListItemExtended> GetReportConfigSelectList(int? tenantFK, string userId, string areaName)
        {
            return CrudReportConfiguration.Read(x =>
                    (x.Lookup.LookupName == "Shared" && x.TenantFk == tenantFK) ||
                    (x.Lookup.LookupName == "Private" && x.UserId == userId))
                        .Where(x => x.Lookup1.LookupName == areaName)
                        .Select(x => new SelectListItemExtended
                        {
                            Text = x.Name,
                            Value = x.ConfigurationValue,
                            HtmlAttributes = new
                            {
                                data_reportConfigId = x.ReportConfigurationId,
                                data_reportConfigName = x.Name,
                                data_canModify = x.UserId == userId,
                                data_extendedConfiguration = x.ExtendedConfiguration
                            }
                        }).ToList();
        }

        public class SelectListItemExtended
        {
