    ...
    public class NetGiant
    {
        ...
        private static void BreakMargins(dynamic extras)
        {
            try
            {
                ...
                if (ruleDetails.Rule.MethodName == "Assembly") return;
        ...
        private static void DoAssembly(dynamic extras)
        {
            try
            {
                ...
                var desAlt2Price = Engine.CalculatePercentDiscount(discountedPrice, ruleDetails.Rule.AltPriceAdj2);
                var desAlt1Price = Engine.CalculatePercentDiscount(discountedPrice, ruleDetails.Rule.AltPriceAdj1);

                if (desAlt2Price > break2MinMarginPrice)
                {
                    ruleDetails.Components.FinalPrice = discountedPrice;
                    ruleDetails.Product.AltPrice2 = desAlt2Price;
                    ruleDetails.Product.AltPrice1 = desAlt1Price;
                    ruleDetails.Components.CalculationOutcome = calculationOutcomes.Find(x => x.LookupName == "Assembly Discount").LookupID;
                }
                else
                {
                    ruleDetails.Product.AltPrice2 = break2MinMarginPrice;

                    if (break2MinMarginPrice < discountedPrice)
                    {
                        ruleDetails.Components.FinalPrice = discountedPrice;
                        ruleDetails.Product.AltPrice1 = Math.Round(((discountedPrice + break2MinMarginPrice) / 2), 2);
                        ruleDetails.Components.CalculationOutcome = calculationOutcomes.Find(x => x.LookupName == "Assembly Discount").LookupID;
                    }
                    else
                    {
                        ruleDetails.Components.FinalPrice = ruleDetails.Product.AltPrice1 = ruleDetails.Product.AltPrice2 = break2MinMarginPrice;
                        ruleDetails.Components.CalculationOutcome = calculationOutcomes.Find(x => x.LookupName == "Minimum").LookupID;
                    }
                }
            }
            catch (Exception e)
        ...
        public static Dictionary<string, string> GetCustomConfig()
        {
            var dict = new Dictionary<string, string>();
            dict.Add("ByPassPriceAdjustments", "true");
