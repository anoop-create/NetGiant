...
namespace DP001BusinessLogic.Pricing
{
    public class Engine
    {
        ...
        private void ProcessRules(IEnumerable<PriceRuleDetail> rules)
        {
            ...
            {
                // Testing usage
                //if (priceRule.Product.ManufacturerPartNo == "59912")
                //{
                //    var xxx = "xxx";
                //}
                var competitors = GetCompetitorPrices(priceRule.Rule);

                if (!ProductHasCostPrice(priceRule))
                {
                    DetermineCompetitivePosition(priceRule, competitors);
                    continue;
                }
                ...
                if (!LookupCustomConfigEntry("ByPassPriceAdjustments", "true"))
                {
        ...
        private List<PriceRuleDetail> GetPriceRules(CalculationMethod method)
        {
            ...
            using (var db = new DP001Entities())
            {
                ...
                CommonDataFunctions.CreateLogEntry(_channel, "END GetPriceRules - " + methodName + ". Product Count = " + results.Count, "Information");
        ...
        private List<PriceRuleDetail> GetCustomPriceRules(string method)
        {
            ...
            using (var db = new DP001Entities())
            {
                ...
                _tenant.CustomConfigSettings = _tenant.CustomConfig?.Invoke();
        ...
        private static DataTable SetupProductDataTable(IEnumerable<PriceRuleDetail> rules)
        {
            ...
            dt.Columns.Add(new DataColumn("CompetitivePosition", typeof(decimal)));
            ...
            foreach (var rule in rules)
            {
                ...
                newRow["CompetitivePosition"] = rule.Product.CompetitivePosition ?? (object)DBNull.Value;
        ...
        private static void OutputPricesToEmail(Channel channel, MemoryStream csv)
        {
            try
            {
                ...
                Email.SendEmail(body, subject, emailTo, "noreply@priceology.netgiant.com", csv, channel.ChannelName);
            ...
            try
            {
                ...
                if (ftpDetail != null)
                {
                    ...
                    var hostDetails = new Ftp.FtpHostDetails
                    {
                        ...
                        SavePath = channel.TenantFK.ToString() + "\\" + ftpDetail.FTPSettingsID + "_" + ftpDetail.FTPFileName
                    };
        ...
        public static MemoryStream CreateInMemoryCsv(List<PriceRuleDetail> priceRules, Channel channel)
        {
            ...
            return CreateCsvFileInMemory(channel);
        }

        private static MemoryStream CreateCsvFileInMemory(Channel channel)
        {
            ...
            {
                CommonDataFunctions.CreateLogEntry(channel, "Could not create in memory csv. ERROR: " + e.StackTrace, "Notification", true);
            }
        ...
        private static void DetermineCompetitivePosition(PriceRuleDetail priceRule, List<decimal> competitors)
        {
            int pos = competitors.Count;
            ...
            {
                pos = competitors.Count + 1;
                foreach (decimal competitorPrice in competitors)
                {
                    if (priceRule.Product.Price > competitorPrice)
                    {
                        break;
                    }
                    pos--;
            ...
            priceRule.Product.CompetitivePosition = pos;
        }
        ...
        private static void CheckProductCounts(Channel channel)
        {
            List<SqlParameter> parms = new List<SqlParameter>();
            SqlParameter parm1 = new SqlParameter("@ChannelID", SqlDbType.Int);
            parm1.Value = channel.ChannelID;
            parms.Add(parm1);

            DataSet ds = SQL.ExecuteReadStoredProcedure("DP001", "CheckCalculationCount", parms);
            DataRow dr = ds.Tables[0].Rows[0];

            string msg = "";
            string type = "Information";
            if (!Convert.ToBoolean(dr["IsSuccess"].ToString()))
            {
                msg = "Count Mismatch: ";
                type = "ScheduleInfo";
            }
            CommonDataFunctions.CreateLogEntry(channel, msg + dr["Message"].ToString(), type);
        }
        ...
        private bool LookupCustomConfigEntry(string configEntry, string configValue)
        {
            if (_tenant.CustomConfigSettings != null)
            {
                if (_tenant.CustomConfigSettings.ContainsKey(configEntry))
                {
                    if (_tenant.CustomConfigSettings[configEntry] == configValue)
                    {
                        return true;
                    }
                }
