...
namespace DP001DataAccess.Utilities
{
    public class Ftp
    {
        ...
        public static bool TestFTPConnection(FtpHostDetails hostDetails)
        {
            try
            {
                if (hostDetails.Protocol == FTPProtocol.FTP || hostDetails.Protocol == FTPProtocol.FTPS)
                {
                    ...
                    var requestDir = InitializeFtpConn(
                        hostDetails.FtpHost,
                        hostDetails.FtpUser,
                        hostDetails.FtpPassword,
                ...
                if (ftpDetails.SourceProtocol == FTPProtocol.FTP || ftpDetails.SourceProtocol == FTPProtocol.FTPS)
                {
                    ...
                    using (var ftpClient = new FtpClient())
                    {
                        ...
                        if (ftpDetails.SourceProtocol == FTPProtocol.FTPS)
                        {
                            ...
                            ftpClient.ValidateCertificate += new FtpSslValidation(CertificateValidation);
                        }
        ...
        private static void CertificateValidation(FtpClient control, FtpSslValidationEventArgs e)
        {
            e.Accept = true;
        } 
        ...
        private static FtpWebRequest InitializeFtpConn(
            string host,
            string username,
            string password,
        ...
        {
            var requestDirectory = (FtpWebRequest)WebRequest.Create(host);
            requestDirectory.Method = requestMethod;
            requestDirectory.Credentials = new NetworkCredential(username, password);

            if (protocol == FTPProtocol.FTPS)
            {
                requestDirectory.EnableSsl = true;
                ServicePointManager.ServerCertificateValidationCallback += (sender, certificate, chain, sslPolicyErrors) => true;
            }
