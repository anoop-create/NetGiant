...
namespace DP001Batch.BusinessObjects.Shared
{
    class StandardFunctions
    {
        ...
        public static void GeneralMaintenance(Dictionary<string, string> parms)
        {
            ...
            if (parms["period"] == "w")
            {
                if (!SQL.ExecuteStoredProcedure("DP001", "WeeklyHousekeeping", new List<SqlParameter>(), 0, 1500))
                {
                    CommonDataFunctions.CreateLogEntry(0, 0, "WeeklyHousekeeping Failed. Problem with SP WeeklyHousekeeping", "Error", true);
                }

                // Cleanup SQL Backups on the Azure Blob Storage
                CommonFunctions.CleanupSqlBackupFiles();
                TruncateTable("CalculationHistory", "Date", -45, Int32.Parse(parms["debug"]));
            }

            if (parms["period"] == "m")
            {
                // Truncate Log, TenantAudit
                TruncateTable("Log", "DateTime", -90, Int32.Parse(parms["debug"]));
                TruncateTable("TenantAudit", "Timestamp", -90, Int32.Parse(parms["debug"]));
                // Delete Old Tenants
            }
        }
