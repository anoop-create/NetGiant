    ...
    public class Scheduling
    {
        ...
        public static void RunSchedule(int slotSize)
        {
            ...
            foreach (var sch in schedules)
            {
                ...
                switch (sch.Lookup.LookupName)
                {
                    case "Load Sales History":
                        ActionSchedule("t$loadsaleshistory,tt$" + tenant.TenantID + ",ch$" + sch.ChannelFK);
                        break;
                    default:
                        ActionSchedule("t$loadinventories,tt$" + tenant.TenantID + ",ch$" + sch.ChannelFK);
                        break;
                }
            }
        }

        private static void ActionSchedule(string command)
        {
            if (ConfigurationManager.AppSettings["Platform"] == "Server")
            {
                string[] args = command.Split(null);
                Dictionary<string, string> dictionary = SwitchDetection.loadParms(args);
                var switchDetect = new SwitchDetection();
                switchDetect.DetectSwitch(dictionary);
            }
            else if (ConfigurationManager.AppSettings["Platform"] == "Azure")
            {
                //AzureFunctions.WriteToAzureStorageQueue(command);
            }
        }
        ...
        public static void RunAdHocSchedule(Dictionary<string, string> parms)
        {
            ...
            if (platform == "Server")
            {
                ...
                foreach (string file in Directory.EnumerateFiles(folderPath, "*.txt"))
                {
                    ...
                    {
                        CommonDataFunctions.CreateLogEntry(0, 0, "Unable to run adhoc routine", "Error", true);
                    }
                }
            }
