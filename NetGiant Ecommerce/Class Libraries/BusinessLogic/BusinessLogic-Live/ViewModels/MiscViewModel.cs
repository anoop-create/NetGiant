...
namespace BusinessLogic.ViewModels
{
    public class MiscViewModel : CommonViewModel
    {
        ...
        private void GetProductsFromDatabase()
        {
            ...
            if (productList == null)
            {
                var productIds = EntityAccess.ReadProduct(x => _stockRefs.Contains(x.AxisField.stockReference)).Select(x => x.productID).ToList();
                var searchModel = new SearchViewModel();
                searchModel.GetProducts(string.Join(",", productIds));
                ProductViewModel.ProductList = searchModel.Products;
                ProductViewModel.ProductList = ProductViewModel.ProductList.OrderBy(x => x.PriceTrIncVat).ToList();
