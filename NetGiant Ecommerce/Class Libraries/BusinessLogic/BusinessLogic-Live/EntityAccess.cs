...
namespace BusinessLogic
{
    public class EntityAccess
    {
        ...
        public static List<product> ReadProduct(Expression<Func<product, bool>> where)
        {
            ...
            try
            {
                using (Ngmd db = new Ngmd())
                {
                    ret = db.products
                        .Where(where)
                        .Include(x => x.AxisField)
                        .Include(x => x.websiteInventories.Select(y => y.productImages))
                        .Include(x => x.productGroup.productType)
                        .Where(where)
                        .ToList();
            ...
            try
            {
                using (Ngmd db = new Ngmd())
                {
                    ret = db.websiteInventories
                        .Include(el => el.product.AxisField.AxisFieldsAdditionals) //??
        ...
        /// <summary>
        /// Read eqCartridgeType entities
        /// </summary>
        /// <param name="where"></param>
        /// <returns></returns>
        public static List<eqCartridgeType> ReadCartridgeType(Expression<Func<eqCartridgeType, bool>> where)
        {
            List<eqCartridgeType> ret = new List<eqCartridgeType>();

            try
            {
                using (Ngmd db = new Ngmd())
                {
                    ret = db.eqCartridgeTypes
                        .Where(where)
                        .OrderBy(x => x.eqCartridgeTypeID)
                        .ToList();
                }
            }
            catch (Exception e)
        ...
        /// <param name="where"></param>
        /// <returns></returns>
        public static List<configurationSetting> ReadConfigurationSetting(Expression<Func<configurationSetting, bool>> where, bool isSiteSpecific = true)
        {
        ...
        /// <summary>
        /// Read product type entities
        /// </summary>
        /// <param name="where"></param>
        /// <returns></returns>
        public static List<productType> ReadProductTypes(Expression<Func<productType, bool>> where)
        {
            List<productType> ret = new List<productType>();
        ...
        public static List<ProductPdf> GetProductPdfs(string partNo, string manufacturerName)
        {
            ...
            {
                pdfDocuments = (from m in db.or_mediaLinks
                    join p in db.or_products on m.prodID equals p.prodID
                    where p.partno == partNo && p.manufacturer == manufacturerName && m.type == "PDF"
                    select new ProductPdf { Description = m.description, Url = m.url + "&v=PD"}).ToList();
            }
        ...
        {
            List<VoucherPromo> ret = new List<VoucherPromo>();
            try
            {
                using (Ngmd db = new Ngmd())
                {
        ...
        public static List<Lookup> ReadLookUp(Expression<Func<Lookup, bool>> where)
        {
        ...
        public static bool InsertCreditAccountApplication(Account account, Customer customer, Billing billing)
        {
            bool success;
