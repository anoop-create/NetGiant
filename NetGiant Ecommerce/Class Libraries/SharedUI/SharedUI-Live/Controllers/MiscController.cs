    ...
    public class MiscController : ApplicationController
    {
        ...
        public JsonResult AskQuestion()
        {
            ...
            else
            {
                ...
                Utilities.SendEmail(Utilities.GetItemFromDict(commondata, "SupportEmail").ToLower(), Utilities.GetItemFromDict(commondata, "QandAEmail").ToLower(), "Ask A Question", "NewQuestionInternal", replacements);
                Utilities.SendEmail(Utilities.GetItemFromDict(commondata, "SupportEmail").ToLower(), qam.Email, "Thank you for your question", "NewQuestionCustomer", replacements);
            }
        ...
        public ActionResult ApplyVoucher(string voucherCode)
        {
            ...
            {
                sr.Html = "<div class=\"\"><i class=\"fa fa-exclamation-triangle fa-lg\"></i><span class=\"g-p-l-10\">Voucher " + voucherCode.ToUpper() + " has been recognised.</span></div>";
            }
        ...
        public ActionResult NgRedirect(string url = "")
        {
            ...
            if (url.ToLower().StartsWith("product/"))
            {
                ...
                // check item exists
                product p = EntityAccess.ReadProduct(x => x.AxisField.stockReference == stockref && (x.productStatusFK == 1 || x.productStatusFK == 8)).FirstOrDefault();
                if (p != null)
                {
