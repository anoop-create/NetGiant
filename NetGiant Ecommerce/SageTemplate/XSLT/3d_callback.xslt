...
<xsl:output method="xhtml" encoding="ISO-8859-1" indent="yes" omit-xml-declaration="yes" media-type="text/html" 
 doctype-public="-//W3C//DTD XHTML 1.0 Transitional//EN" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"/>
	...
	<xsl:template match="/">
		...
		This page appears inside the inline frame and automatically redirects the customer back to the Sage Pay
		authorisation pages.  It only displays if Javascript is turned off with a simple message asking the user to click
		...
		<html>
			<head>
				<title><xsl:value-of select="$i18n/multi-lingual-text/texts[lang($requiredlang)]/text[@label='threed_d_callback_tittle']" /></title>
				<style type="text/css" media="screen">
				@import url("<xsl:copy-of select="$imageloc"/>vsp3.css");
				</style>
			</head>
			<body onload="OnLoadEvent();">
				<form name="form" action="" method="POST" target="_top">
					<xsl:attribute name="action"><xsl:value-of select="payment-model/threedcompletion-url"/></xsl:attribute>
					<input type="hidden" name="PaRes">
						<xsl:attribute name="value"><xsl:value-of select="payment-model/PaRes"/></xsl:attribute>
					</input>
					<input type="hidden" name="MD">
						<xsl:attribute name="value"><xsl:value-of select="payment-model/MD"/></xsl:attribute>					
					</input>					
